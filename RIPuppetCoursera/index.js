...
console.log(inputValues);
  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      await page.type('input[id="identification"]', 'c.zapatat@uniandes.edu.co');
      await page.type('input[id="password"]', '1234qwerty');
      ...
      fs.copyFileSync('./index.html', `${basePath}/index.html`);
...
async function scrapLinks(page){
  const stories = await page.evaluate(() => {
    ...
    return uniqueLinks = [...new Set(webLinks)].filter(link => link.includes(':2368'));
    });
