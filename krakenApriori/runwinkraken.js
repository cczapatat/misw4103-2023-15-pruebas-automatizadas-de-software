...
function runWorker(fileIndex) {
    pathrel = path.resolve("features",`scenario_apriori_${fileIndex}.feature`);  
    const currentFileName = pathrel;
  for (var i=fileIndex+1; i<= 20; i++){
    try{

        pathrel2 = path.resolve("features",`scenario_apriori_${i}.feature`); 
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    try {
      pathrel3 = path.resolve("features",`scenario_apriori_${fileIndex}.feature`); 
    ...
    for (var i=fileIndex+1; i<= 20; i++){
        try {
          pathrel4 = path.resolve("features",`scenario_apriori_${i}.featured`); 
...
function restoreFilenames(){
    for (var i=0; i<= 20; i++){
        try {
          pathrel5 = path.resolve("features",`scenario_apriori_${i}.featured`); 
